...
def create_reply(db: Session, message_content: str, reply_to_id: int, user_id: int):
    reply_content = chatbot.generate_reply(message_content)
    new_reply = models.Message(content=reply_content, reply_to=reply_to_id, user_id=user_id)
    db.add(new_reply)
    db.commit()
...
def delete_message_by_id(db: Session, message_id: int, user_id: int):
    message = get_message_by_id(db, message_id, user_id)
    db.delete(message)
    db.commit()
    return message

def update_message(db: Session, message_id: int, user_id: int, content: str):
    message = get_message_by_id(db, message_id, user_id)
